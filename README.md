...
- **Subscriptions you already pay for** — Claude Code, Codex, Gemini CLI,
  Cursor, OpenCode, GitHub Copilot, Hermes, IBM Bob, Prime Agent, and Pi run
...
> embedded vector + relational storage, and a **Linear-inspired** UI with 37 themes.
...
- **Search your Mac** — don't remember where a file lives? Type a few characters
  in Add Source and a Spotlight-backed search returns ranked file and folder
  hits, addable in one click. Ingestible, name-matching, recently-modified files
  float to the top, and Trash / `node_modules` / `~/Library` noise is filtered
  out. Adds route through the same path as a drag-and-drop, so folders become
  synced folder sources and OKF bundles import exactly as they would otherwise.
...
- **Slash commands** — type `/` in the chat composer to open a picker over every
  ...
  completes, and an unrecognized slash just sends as ordinary text. `/grep` runs
...
- **37 themes** — Acid Garage, Blueprint, Camo Ice, Camo Jungle, Camo Sand,
  Camo Urban, Carrera, Carrera Dark, Catppuccin Latte, Claude, Dracula,
  Durandal, GitHub, GitHub Light, Gruvbox, IBM Carbon, IBM Carbon Light,
  Italia, Light, Matrix, Midnight, Monokai, Night City, Nord, One Dark,
  OpenAI, Paddock, Panigale, QDOS, Rosé Pine Dawn, Sakura, Sepia, Slate,
  Solarized, Solarized Light, Synthwave '84, Tokyo Night.
- **Themed design elements** — themes with strong iconography restyle the
  ...
  tachometer, Italia a slipstream at speed, Panigale a trellis lattice), open the hero on their
...
- **Follows your system text size** — the entire interface scales to the macOS
  Accessibility text-size setting (System Settings → Accessibility → Display),
  re-reading it on window focus, since a webview otherwise ignores it outright.
  Your own chat and reader size preferences compose on top of it, and PDF export
  is pinned so printed pages never scale.
