...
| **Agent CLIs** | claude (Max), codex (ChatGPT Pro), opencode, hermes, copilot, gemini-cli, prime-agent, pi | the vendor's own CLI holds the subscription |
...
3. **Agent CLIs** — claude + codex adapters first (the two with proven
   ...
   `--append-system-prompt`). pi (earendil-works/pi, prime-agent's
   upstream) followed the same day — identical protocol, shared parse arm.
