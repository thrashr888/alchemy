Every answer in Alchemy is built from numbered excerpts pulled from your actual sources, and the citations under an answer point at the exact passages used. This source explains the loop and the tools around it.

## How an answer happens

When you ask a question, Alchemy searches your selected sources two ways — by meaning and by exact keywords — and merges the results, so it catches both "means the same thing" matches and exact-term matches like part numbers or names. The best excerpts are assembled into a numbered prompt, your chat model streams an answer, and the citations are saved with the message.
...
- **Source checkboxes** in the left rail control which sources are searched.
- **@ mentions** — type @ to name a source, folder, or note; that one question searches only what you named.
...
- **Deep research** — runs several searches in sequence for questions a single search can't answer well.
