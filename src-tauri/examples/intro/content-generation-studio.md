...
- **Summary, FAQ, Study guide, Briefing, Timeline** — the workhorses for getting oriented in a notebook.
...
One click turns the notebook into a two-host podcast episode. Your chat model writes the script; the voices are synthesized **entirely on-device** by Kokoro neural TTS (a one-time download of roughly 93 MB). The script stays readable and editable as a note, and the episode plays inline. It is a good way to absorb a notebook while doing something else.
...
- **Convert to source** folds a note into the source set, so your own writing becomes searchable and citable like anything else.
