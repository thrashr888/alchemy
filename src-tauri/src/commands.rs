...
pub(crate) fn e<T>(r: anyhow::Result<T>) -> Result<T, String> {
...
pub async fn source_hygiene(
    ...
    let shared = e(state.db.sources_with_content_shared(&notebook_id).await)?;
    // A source whose origin lives on another Mac is not a missing file (§5.8);
    // marking needs an owned slice, which is one clone only while the cache
    // still holds the scan.
    let mut sources = std::sync::Arc::unwrap_or_clone(shared);
    crate::device::mark_remote(&app_data_dir(&state), &notebook_id, &mut sources);
...
async fn rescan_one_folder_inner(
            ...
            None => match extract_any_file(state, path).await {
                Ok(mut extracted) => {
                    let settled = (folder.source_type == "okf"
                        && crate::okf::okf_title_from_frontmatter(&mut extracted))
    ...
    if folder.source_type == "okf" {
        crate::okf::refresh_okf_lifecycle(state, folder).await;
    }
    Ok(scan)
}
...
pub async fn index_note(state: &AppState, note: &Note) {
    ...
    // notebook's bundle write is scheduled (docs/RFC-okf-live.md §5.2).
...
pub async fn delete_notes(
    ...
    // Which notebooks these belonged to, read before the rows go: a bound
    // notebook's bundle has to lose the files too (RFC-okf-live §5.2).
    let mut owners: HashSet<String> = HashSet::new();
    for id in &ids {
        if let Ok(Some(note)) = state.db.get_note(id).await {
            owners.insert(note.notebook_id);
        }
    ...
    for owner in &owners {
        crate::okf::schedule_write(owner);
    }
    Ok(())
}
...
pub(crate) fn find_bundle_root(dir: std::path::PathBuf) -> Result<std::path::PathBuf, String> {
